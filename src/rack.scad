...
_d_w_t = damage_t + rounding;
_d_w_w = damage_w + 2*rounding;
_d_w_h = damage_h + 2*rounding;
_initiative_working_h = initiative_h + 2 *rounding;
_initiative_working_w = initiative_w + 2 *rounding;
_initiative_working_t = initiative_t + rounding;
...
module rack(total_height=h_working) {
    ...
    difference() {
        ...
        {
            translate([(l_working-_initiative_working_h)/2, (w_working - _initiative_working_w)/2 , total_height-_initiative_working_t])
            cube([_initiative_working_h, _initiative_working_w, _initiative_working_t]);
            // damage counter cutout on top

            translate([(l_working-_d_w_h)/2 + 5,(w_working-_d_w_w)/2,total_height])
